...
def format_report_index(reports, trigger_api_url):
    ...
    return (
        ...
        "          <p class=\"mt-2 text-xs text-slate-500\">Automatic checks run at 14:00, 16:00, and 18:00 Vilnius time until new data appears.</p>\n"
        ...
        "          <div id=\"report-content\"><p class=\"text-center text-slate-400 text-sm py-16 px-4\">Tap &ldquo;Generate for this device&rdquo; to load reports on this browser.</p></div>\n"
        "        </div>\n"
        "      </div>\n"
        "    </section>\n"
        "  </main>\n"
        ...
        "const AUTO_CHECK_SLOTS = [14 * 60, 16 * 60, 18 * 60];\n"
        ...
        "  if (totalMinutes >= AUTO_CHECK_SLOTS[2]) return AUTO_CHECK_SLOTS[2];\n"
        "  if (totalMinutes >= AUTO_CHECK_SLOTS[1]) return AUTO_CHECK_SLOTS[1];\n"
        "  if (totalMinutes >= AUTO_CHECK_SLOTS[0]) return AUTO_CHECK_SLOTS[0];\n"
        "  return null;\n"
        "}\n"
        ...
        "    const currentDates = readVisibleDates();\n"
        "    const visibleSet = new Set(currentDates);\n"
        "    let added = 0;\n"
        ...
        "    state.found = true;\n"
        "    writeAutoCheckState(state);\n"
        "    saveVisibleDates(Array.from(visibleSet));\n"
        ...
        "  btn.disabled = true;\n"
        "  // First visit: this device has no visible dates yet — just adopt all published reports immediately, no CI trigger needed.\n"
        "  const currentVisible = readVisibleDates();\n"
        "  if (!currentVisible.length) {\n"
        "    const allDates = getGlobalSortedReports().map(function(r){ return r.date; });\n"
        "    saveVisibleDates(allDates);\n"
        "    btn.disabled = false;\n"
        "    btn.textContent = 'Check for updates';\n"
        "    window.location.reload();\n"
        "    return;\n"
        "  }\n"
        "  // Returning device: trigger CI and wait for a genuinely newer date.\n"
        ...
        "    showNotice('Workflow triggered. Waiting for publish...', 'info');\n"
        ...
        "  });\n"
        "  // Adjust button label based on whether this device already has reports.\n"
        "  const isFirstVisit = !readVisibleDates().length;\n"
        "  if (btn) btn.textContent = isFirstVisit ? 'Load on this device' : 'Check for updates';\n"
        "  if (!sorted.length) {\n"
        "    document.getElementById('report-content').innerHTML = '<p class=\"text-slate-500 text-sm text-center py-16\">No reports on this device yet.<br>Tap <strong>Load on this device</strong> to adopt the latest published data.</p>';\n"
        "  } else {\n"
