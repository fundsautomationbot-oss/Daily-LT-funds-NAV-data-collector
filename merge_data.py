...
def main():
    ...
    try:
        ...
        html_content = (
            ...
            "  notification.className = `fixed top-4 right-4 p-4 rounded-2xl border ${bgClass} shadow-lg z-50 max-w-sm animate-pulse`;\n"
            ...
            "      showNotification('Could not find available reports', 'warning');\n"
            "      btn.textContent = '🔄 Check for updates';\n"
            "      btn.disabled = false;\n"
            "      return;\n"
            "    }\n"
            "    const latestDate = dateMatches[dateMatches.length - 1].match(/\\d{4}-\\d{2}-\\d{2}/)[0];\n"
            "    if (latestDate === currentReportDate) {\n"
            "      showNotification('✓ Latest data is already displayed', 'success');\n"
            "    } else {\n"
            "      showNotification(`✓ New data available (${latestDate})! Reloading...`, 'success');\n"
            "      setTimeout(() => window.location.reload(), 1500);\n"
            "    }\n"
            "  } catch (error) {\n"
            "    showNotification('Error checking for updates: ' + error.message, 'warning');\n"
            "  } finally {\n"
            "    if (!btn.disabled) {\n"
            "      btn.textContent = '🔄 Check for updates';\n"
            "      btn.disabled = false;\n"
