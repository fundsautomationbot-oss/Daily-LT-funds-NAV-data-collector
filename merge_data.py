...
def format_report_index(reports, trigger_api_url):
    ...
    return (
        ...
        "            <select id=\"report-select\" class=\"w-full rounded-full border border-slate-300 bg-slate-50 px-4 py-2.5 text-sm font-semibold text-slate-700 shadow-sm outline-none transition focus:border-slate-500 focus:ring-2 focus:ring-slate-200\"></select>\n"
            "            <button id=\"check-updates-btn\" class=\"w-full sm:w-auto justify-center rounded-full border border-slate-400 bg-white px-4 py-2.5 text-sm font-semibold text-slate-700 transition hover:bg-slate-100\">Load published data</button>\n"
        ...
        "    const published = (await fetchPublishedReports()).sort(byNewest);\n"
        "    if (!published.length) {\n"
        "      showNotice('No published reports found at the moment.', 'warning');\n"
        "      return;\n"
        "    }\n"
        ...
        "      return;\n"
        "    }\n"
        "    showNotice('Already up to date with published data.', 'info');\n"
        "  } catch (e) {\n"
        "    const msg = (e && e.message) ? e.message : 'network error';\n"
        "    showNotice('Could not sync published data: ' + msg, 'error');\n"
        "  } finally {\n"
        "    btn.disabled = false;\n"
        "    const isFirstVisit = !readVisibleDates().length;\n"
        "    btn.textContent = isFirstVisit ? 'Load published data' : 'Sync published data';\n"
        "  }\n"
        "}\n"
        "function initIndex() {\n"
        "  const btn = document.getElementById('check-updates-btn');\n"
        ...
        "  const isFirstVisit = !readVisibleDates().length;\n"
        "  if (btn) btn.textContent = isFirstVisit ? 'Load published data' : 'Sync published data';\n"
        "  if (!sorted.length) {\n"
        "    document.getElementById('report-content').innerHTML = '<p class=\"text-slate-500 text-sm text-center py-16\">No reports on this device yet.<br>Tap <strong>Load published data</strong> to adopt the latest published data.</p>';\n"
        "  } else {\n"
