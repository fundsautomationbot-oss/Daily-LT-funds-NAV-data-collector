...
def format_report_index(reports, trigger_api_url):
    ...
    return (
        ...
        "          <p id=\"last-check-note\" class=\"mt-2 text-xs text-slate-500\">Last automatic check: not yet</p>\n"
        "        </div>\n"
        ...
        "const NEXT_PUBLISHED_CHECK_KEY = 'ltFundsNextPublishedCheckAt';\n"
        ...
        "  localStorage.setItem(NEXT_PUBLISHED_CHECK_KEY, String(Date.now() + AUTO_CHECK_INTERVAL_MS));\n"
        ...
        "    return;\n"
        "  }\n"
        "  const nextCheckAt = Number(localStorage.getItem(NEXT_PUBLISHED_CHECK_KEY) || '0');\n"
