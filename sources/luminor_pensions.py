...
try:
page.wait_for_selector("text=Unit
value", timeout=10000)
break
except:
if attempt == 2:
...
data_date = None
m = re.search(
r"(?:Unit value date|Vieneto verciy data)
[^d]*(1d{4}-ld{2}-1d(2})",
body
)
if m:
...
continue
unit_value_match = re.search(r"Unit
value\s+([\d.,]+)", text)
nav_match = re.search(r"Net asset
value|s+([ld\s.,]+)", text)
if not unit_value_match or not nav_match:
continue
unit_value =
unit_value_match.group(1).replace(",", "")
net_assets =
nav_match.group(1).replace(' ", '')
results.append ({
"Fund name": fund_name,
"Data": data_date,
"Vieneto verté": unit_value,
